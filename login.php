...
if (isset($_POST['login'])) {
    if (isset($_POST['email']) && isset($_POST['password'])) {
        ...
        $result = mysqli_query($con, "SELECT * FROM user WHERE email='$user_login' AND activation='yes'");
        $num = mysqli_num_rows($result);
        $get_user_email = mysqli_fetch_assoc($result);
        if ($num > 0) {
            if (password_verify($password_login, $get_user_email['password'])) {
            ...
            $result1 = mysqli_query($con, "SELECT * FROM user WHERE email='$user_login' AND activation='no'");
